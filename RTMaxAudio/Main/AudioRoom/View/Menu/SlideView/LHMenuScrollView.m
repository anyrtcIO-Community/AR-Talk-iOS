...
static const CGFloat kMenuTitleNormalFontSize = 15.f;           // 默认按钮未选中的字体大小
static const CGFloat kMenuTitleSelectedFontSize = 18.f;         // 默认按钮选中的字体大小
...
//    return;
//    if (_menuButtonArray) {
//        for (UIButton *button in _menuButtonArray) {
...
//            [self changeColorForButton:menu_btn red:1];
            [self selectedButton:menu_btn percent:1];
...
-(void)initSlideView
{
    ...
    _slideView.backgroundColor = [RMCommons getColor:@"#22C485"];// kScrollLineViewColor;
...
- (void)changeTitlesFontSizeByScrollViewOffsetX:(CGFloat)contentOffsetX
{
    ...
    [self selectedButton:_firstButton percent:(1-percent)];
    // 最后一个 防止越界
...
//    [self updateButtonState:_secondButton percent:percent];
    [self unselectedButton:_secondButton percent:percent];
    
}

- (void)selectedButton:(UIButton *)button percent:(CGFloat)percent {
    ...
    [button setTitleColor:[RMCommons getColor:@"#22C485"] forState:UIControlStateNormal];
}
- (void)unselectedButton:(UIButton*)button percent:(CGFloat)percent {
    CGFloat value = [self lerp:percent min:kMenuTitleNormalFontSize max:kMenuTitleSelectedFontSize];
    button.titleLabel.font = [UIFont boldSystemFontOfSize:value];
    [button setTitleColor:[RMCommons getColor:@"#999999"] forState:UIControlStateNormal];
}
